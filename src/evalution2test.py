...
Two independent scoring axes per answer:
    1. Reason score (0.0 / 0.5 / 1.0) via LLM judge rubric.
    2. Numeric error (absolute + relative) — only where both the verified
       answer and the model's MAGNITUDE field contain an actual number.
"""
...
JUDGE_MODEL = "claude-sonnet-4-5"   # confirm exact model string before locking
...
class ScoredAnswer(TypedDict):
    ...
    leaked: Optional[bool]
    judge_reasoning: str
    numeric_error: Optional[NumericError]
    scorer: str
...
Additionally, extract numeric values if and only if BOTH the verified correct answer AND the test AI's MAGNITUDE field contain an actual number or numeric range. If either side is categorical with no genuine figure, output N/A for all numeric fields. Do not invent or estimate a number that was not actually stated. If the AI gave a single point value rather than a range, use that same value for both LOW and HIGH.
...
class AnswerJudge:
    ...
    def score(self, question: QuestionRecord, raw_answer: str, condition: str) -> ScoredAnswer:
    ...
    def _check_leakage(self, question: QuestionRecord, raw_answer: str, condition: str) -> ScoredAnswer:
...
def run_evaluation(
    ...
    for i, question in enumerate(questions, 1):
        logger.info("[%d/%d] Processing %s (%s)", i, len(questions), question["id"], question["question_type"])
        ...
        for condition, result in generation_results.items():
            ...
            if scored["leaked"] is not None:
                logger.info("  %s: LEAKAGE — %s", condition, "LEAKED" if scored["leaked"] else "clean")
            else:
                numeric_note = ""
...
def compute_improvement_scores(scored_results: list[ScoredAnswer]) -> dict[str, dict[str, float]]:
...
def aggregate_numeric_errors(scored_results: list[ScoredAnswer]) -> dict[str, dict]:
...
def summarize_improvements(improvements: dict[str, dict[str, float]]) -> dict[str, dict]:
...
def cochrans_q_test(
    ...
    complete = {qid: scores for qid, scores in by_question.items() if all(c in scores for c in CONDITIONS)}
